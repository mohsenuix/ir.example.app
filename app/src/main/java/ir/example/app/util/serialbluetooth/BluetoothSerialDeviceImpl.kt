...
internal class BluetoothSerialDeviceImpl constructor(
    ...
    override fun openMessageStream(): Flowable<ByteArray> {
        ...
        return Flowable.create({ emitter ->
            val reader = CustomBufferedReader(InputStreamReader(inputStream, charset))
            while (!emitter.isCancelled && !closed.get()) {
                synchronized(inputStream) {
                    try {
                        val receivedString = reader.readNextPacket()
                        if (!TextUtils.isEmpty(receivedString)) {
                            emitter.onNext(receivedString.toByteArray())
                        }
                    } catch (e: Exception) {
